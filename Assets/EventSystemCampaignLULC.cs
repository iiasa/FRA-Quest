...
public class EventSystemCampaignLULC : MonoBehaviour {
	...
	public GameObject m_ButtonLCCorrectSel;
	public GameObject m_ButtonLCWrongSel;
	public GameObject m_ButtonLUCorrectSel;
	public GameObject m_ButtonLUWrongSel;
	...
	void Start () {
		...
		m_ButtonLCCorrectSel.SetActive (false);
		m_ButtonLCWrongSel.SetActive (false);
		m_ButtonLUCorrectSel.SetActive (false);
		m_ButtonLUWrongSel.SetActive (false);
	}
	...
	public void updateStates() {
		...
		if (Application.systemLanguage == SystemLanguage.German || true){
			text.text =  LocalizationSupport.GetString("LULCTitle");//"Warum ist der Punkt nicht erreichbar?";

			text = m_TextFunction.GetComponent<UnityEngine.UI.Text>();
			text.text = LocalizationSupport.GetString("LULCLU");//"Grund:";
			...
			text.text =  LocalizationSupport.GetString("LULCLC");//"Grund:";
	...
	public void OnQuestion1Click() {
		string[] options2 = { "OK" };
		messageBox.Show ("", LocalizationSupport.GetString("LULCQuestion1"), options2);	
	}
	public void OnQuestion2Click() {
		string[] options2 = { "OK" };
		messageBox.Show ("", LocalizationSupport.GetString("LULCQuestion2"), options2);	
	}
	...
	{
		if (m_LCCorrect == 1) {
			m_ButtonLCCorrectSel.SetActive (true);
			m_ButtonLCWrongSel.SetActive (false);
		} else if (m_LCCorrect == 0) {
			m_ButtonLCCorrectSel.SetActive (false);
			m_ButtonLCWrongSel.SetActive (true);
		}

		if (m_LUCorrect == 1) {
			m_ButtonLUCorrectSel.SetActive (true);
			m_ButtonLUWrongSel.SetActive (false);
		} else if (m_LUCorrect == 0) {
			m_ButtonLUCorrectSel.SetActive (false);
			m_ButtonLUWrongSel.SetActive (true);
		}

		if (m_LCCorrect != -1 && m_LUCorrect != -1) {
			m_Button.SetActive (true);
		}
	}
	...
	int m_LCCorrect = -1;
	int m_LUCorrect = -1;
	public void OnLCCorrect() {
		m_LCCorrect = 1;
		answerUpdated ();
	}
	public void OnLCWrong() {
		m_LCCorrect = 0;
		answerUpdated ();
	}
	public void OnLUCorrect() {
		m_LUCorrect = 1;
		answerUpdated ();
	}
	public void OnLUWrong() {
		m_LUCorrect = 0;
		answerUpdated ();
	}
	...
	public void CampaignFinished() {
		...
		PlayerPrefs.SetInt ("Quest_" + m_NrQuestsDone + "_" + "CampaignType", 2);
