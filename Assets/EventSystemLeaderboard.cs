...
public class EventSystemLeaderboard : MonoBehaviour {
	...
	public void updateStates() {
		...
		/*if (Application.systemLanguage == SystemLanguage.German ) {
			...
			m_Rank.GetComponentInChildren<UnityEngine.UI.Text>().text = LocalizationSupport.GetString("LeaderboardRank");//"Rang";
			m_LName.GetComponentInChildren<UnityEngine.UI.Text>().text =  LocalizationSupport.GetString("LeaderboardName");//"Name";
			m_Score.GetComponentInChildren<UnityEngine.UI.Text>().text =  LocalizationSupport.GetString("LeaderboardScore");//"Quests gemacht";
...
//			UnityEngine.UI.Dropdown.OptionData list3 = new UnityEngine.UI.Dropdown.OptionData("Last week");
		...
		//		dropdown.options.Add (list3);
		//}
		m_ListSampleIds = new List<string>();
		m_ListSampleIds.Add("-1");
		...
		for (int i = 0; i < splitArray.Length; i++)
		{
			...
			if (name.Length > 0)
			{
				Debug.Log("Campaign name: " + name);
				// Find name
				int nrsessions = PlayerPrefs.GetInt("NrValidationSessions");
				bool bFound = false;
				for (int vali = 0; vali < nrsessions && !bFound; vali++)
				{
					if (PlayerPrefs.GetString("Session_" + vali + "_SampleId") == splitArray[i])
					{
						name = PlayerPrefs.GetString("Session_" + vali + "_SampleName");
						bFound = true;

						m_ListSampleIds.Add(PlayerPrefs.GetString("Session_" + vali + "_SampleId"));
					}
				}
				if (!bFound)
				{
				...
				list = new UnityEngine.UI.Dropdown.OptionData(name);
				dropdown.options.Add(list);
			}/* else
            {
				Debug.Log("Campaign no name");
            }*/
		}
	...
	{
		string url = "https://geo-wiki.org/Application/api/Campaign/FRAQuestStats";
		string param = "";
	...
	/*	if (m_CurSelection == 0) {
		...
		//if (m_CurSelection == 0) {
			param += "{\"scope\":\"" + "total" + "\",\"limit\":\"" + "-1" + "\""+ ",\"platform\":{" + "\""+ "app"+ "\":" + "\"21\"" + "}";
		/*} else if (m_CurSelection == 1) {
			param += "{\"scope\":\"" + "thisweek" + "\",\"limit\":\"" + "-1" + "\""+ ",\"platform\":{" + "\""+"app"+ "\":"  + "\"21\"" + "}";
		} else if (m_CurSelection == 2) {
			param += "{\"scope\":\"" + "lastweek" + "\",\"limit\":\"" + "-1" + "\""+ ",\"platform\":{" + "\""+ "app"+ "\":" + "\"21\"" + "}";
		}*/


		UnityEngine.UI.Dropdown dropdown;
		dropdown = m_Dropdown.GetComponent<UnityEngine.UI.Dropdown>();
		string sampleid = m_ListSampleIds[dropdown.value];
		Debug.Log("Dropdown value: " + dropdown.value + " sampleid: " + sampleid);

		if(dropdown.value != 0)
        {
			param = "{\"scope\":\"" + "total" + "\", \"sampleid\":" + sampleid + ",\"limit\":\"" + "-1" + "\"" + ",\"platform\":{" + "\"" + "app" + "\":" + "\"21\"" + "}";
		} else
        {
			param = "{\"scope\":\"" + "total" + "\", \"sampleid\":" + -1 + ",\"limit\":\"" + "-1" + "\"" + ",\"platform\":{" + "\"" + "app" + "\":" + "\"21\"" + "}";
		...
		Debug.Log ("leaderboard param: " + param);
	...
	public void OnSelectedRange( int value) {
		...
		string sample_id = "";
		if (value >= 0 && value < m_ListSampleIds.Count)
		{
			sample_id = m_ListSampleIds[value];
		}

		Debug.Log ("OnSelected: " + value + "," + dropdown.value + ", sample_id: " + sample_id);
