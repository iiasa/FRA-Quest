...
public class EventSystemDebugSelectPin : MonoBehaviour {
	...
	public GameObject m_ImageBackSave;
	public GameObject m_ImageBackTextSave;
	public GameObject m_ImageTextTitleSave;
	...
	void Start () {
		...
		m_ImageBackSave.SetActive (false);
		m_ImageBackTextSave.SetActive (false);
		m_ImageTextTitleSave.SetActive (false);
	...
	public void updateStates() {
		...
		if (Application.systemLanguage == SystemLanguage.German || true){
			text.text =  "Select the campaign type (Selection will not be in final version)";//LocalizationSupport.GetString("ReportTitle");//"Warum ist der Punkt nicht erreichbar?";

			text = m_TextReason.GetComponent<UnityEngine.UI.Text>();
			text.text =  LocalizationSupport.GetString("NotInReachReason");//"Grund:";

			m_Button.GetComponentInChildren<UnityEngine.UI.Text> ().text = "SELECT";// LocalizationSupport.GetString("Report");//"WEITER";
			m_ButtonBack.GetComponentInChildren<UnityEngine.UI.Text> ().text =  LocalizationSupport.GetString("Back");//"Zurück";
			m_BtnSave.GetComponentInChildren<UnityEngine.UI.Text> ().text =  LocalizationSupport.GetString("Next");
			m_ImageTextTitleSave.GetComponentInChildren<UnityEngine.UI.Text> ().text =  LocalizationSupport.GetString("ReportSaved");

			m_Combobox.GetComponentInChildren<UnityEngine.UI.Text>().text = "< Select >";//"< Grund auswählen >";

			UnityEngine.UI.Dropdown.OptionData list = new UnityEngine.UI.Dropdown.OptionData( "< Select >");
			//UnityEngine.UI.Dropdown.OptionData list2 = new UnityEngine.UI.Dropdown.OptionData( LocalizationSupport.GetString("ReportSelectChange"));
			UnityEngine.UI.Dropdown.OptionData list3 =  new UnityEngine.UI.Dropdown.OptionData( "Building campaign");
			UnityEngine.UI.Dropdown.OptionData list4 =  new UnityEngine.UI.Dropdown.OptionData( "Flash campaign");
			UnityEngine.UI.Dropdown.OptionData list5 =  new UnityEngine.UI.Dropdown.OptionData( "Hotspots campaign");
			UnityEngine.UI.Dropdown.OptionData list6 =  new UnityEngine.UI.Dropdown.OptionData( "LULC campaign");
			//new UnityEngine.UI.Dropdown.OptionData(LocalizationSupport.GetString("ReportBuildingAdded"));/*
	...
	public void NextClicked () {
		PlayerPrefs.SetInt ("CampaignSelected", m_CurSelection);
		PlayerPrefs.SetInt ("CurQuestReached", 1);
		PlayerPrefs.Save ();

		Debug.Log ("m_CurSelection: " + m_CurSelection);
		if (m_CurSelection == 1) {
			Application.LoadLevel ("CampaignBuilding");
...
//			Application.LoadLevel ("CampaignFlash");
		...
		/*
		m_ImageBackSave.SetActive (true);
		m_ImageBackTextSave.SetActive (true);
		m_ImageTextTitleSave.SetActive (true);
